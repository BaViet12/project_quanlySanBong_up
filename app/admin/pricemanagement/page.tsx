  ...
  return (
    <div className='p-3'>
      <div className='flex justify-between items-center'>
        <h1 className='text-2xl font-bold'>Quản lý giá sân bóng</h1>
        <button className="bg-green-800 p-3 text-white rounded-md hover:bg-cyan-600" onClick={()=>document.getElementById('my_modal_3').showModal()}>Thêm giá sân</button>
            <dialog id="my_modal_3" className="modal">
              <div className="modal-box">
                <form method="dialog">
                  <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                </form>
                <h3 className='font-bold text-lg mb-4'>{isEditing ? "Cập nhật giá sân" : "Thêm mới giá sân"}</h3>
                <form method='dialog' onSubmit={handleSubmit}>
                    ...
                    <div className='mb-4'>
                        ...
                        <input type="text" name='name' value={formPrice.name} onChange={handleChange} className="w-full px-3 py-2 border rounded" required/>
                    </div>
                    <div className='mb-4'>
                        <label className='block text-gray-700'>Mã sân</label>
                        <input type="number" name='field_id' value={formPrice.field_id} onChange={handleChange} className="w-full px-3 py-2 border rounded" required />
                    </div>
                    <div className='mb-4'>
                        <label className='block text-gray-700'>Mã giờ</label>
                        <input type="number" name='timeslot_id' value={formPrice.timeslot_id} onChange={handleChange} className="w-full px-3 py-2 border rounded" required />
                    </div>
                    <div className='mb-4'>
